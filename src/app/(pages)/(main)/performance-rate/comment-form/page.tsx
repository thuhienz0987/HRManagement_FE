  ...
  useEffect(() => {
    const getEmployees = async () => {
      ...
      if (session?.user.roles.includes(process.env.DepartmentManager))
        url = "/leaders-without-comments/" + session?.user.departmentId._id;
      if (session?.user.roles.includes(process.env.CEO))
        url = "/managers-without-comments/";
      try {
        const res = await axiosPrivate.get<Employee[]>(url + selectedMonth, {
    ...
    const getCommentByMonth = async () => {
      try {
        const res = await axiosPrivate.get<dCommentGet[]>(
          "/comments-by-reviewerId/" + session?.user._id + selectedMonth
        );
          ...
          <div className="flex w-full items-start py-5 gap-12">
            <div>
              ...
              {errors.revieweeId && touched.revieweeId && (
                <span className={errorClassName}>{errors.revieweeId}</span>
              )}
            </div>
            ...
            <div className="flex flex-1 flex-col gap-1">
              ...
              {errors.rate && touched.rate && (
                <span className={errorClassName}>{errors.rate}</span>
              )}
            </div>
          </div>
