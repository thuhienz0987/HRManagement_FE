...
const SalaryDetails = ({
  ...
  useEffect(() => {
    const getSalary = async (_id: string) => {
      try {
        const res = await axiosPrivate.get<dSalary>("/salary/" + _id);
        console.log(res.data);
        res.data.rate = res.data.idComment ? res.data.idComment.rate : 0;
        res.data.comment = res.data.idComment
          ? res.data.idComment.comment
          : "There are no comments yet";
        res.data.allowanceName = res.data.idAllowance.map(
          (allowance) => allowance.name
        );
        res.data.allowanceMoney = res.data.idAllowance.map(
          (allowance) => allowance.amount
        );
        setIdAllowance(
          Array.from(
            new Set(res.data.idAllowance.map((allowance) => allowance._id))
          )
        );
        setSalary(res.data);
      } catch (e) {
    ...
    const getAllowances = async () => {
      try {
        const res = await axiosPrivate.get<dAllowances[]>("/allowances");
  ...
  useEffect(() => {
    ...
    idAllowance?.map((id) => {
      allowances?.map(
        (allowance) =>
          allowance._id === id && allowanceName.push(allowance.name)
      );
    });
    ...
    if (idAllowance && allowances && salary) {
      ...
      totalIncomeMoney =
        parseFloat(salary.dayMoney) +
        totalAllowanceMoney +
        parseFloat(salary.bonusMoney) +
        parseFloat(salary.overTimeMoney) +
        parseFloat(salary.overTimeDayMoney) +
        parseFloat(salary.paidLeaveDaysMoney);
      ...
      <div className=" w-11/12 rounded-lg flex bg-white  dark:bg-dark flex-col md:flex-row">
            ...
            <div className="flex text-[#5B5F7B] dark:text-whiteOff gap-10 flex-col mt-7 ml-7">
              ...
              {allowances?.map((allowance) => (
                ...
                >
                  <Checkbox
                    color="warning"
                    isSelected={idAllowance?.includes(allowance._id)}
                    onChange={(event) =>
                      handleCheckboxChange(event, allowance._id)
                    }
                  />
        ...
        <div className="flex flex-[2_2_0%] flex-col">
          <div className="md:grid flex flex-col grid-cols-1 md:grid-cols-2 md:grid-flow-row w-full pt-16 pb-3 px-5 gap-y-5 gap-x-7">
            <div className="flex text-[#5B5F7B] dark:text-whiteOff gap-10">
              <p className="inline text-start break-words font-semibold">
                Full name:
              </p>
            ...
            <div className="flex text-[#5B5F7B] dark:text-whiteOff gap-10">
              ...
              <p className="text-start font-normal inline">
                {salary?.userId.departmentId.name}
              </p>
            </div>
            <div className="flex text-[#5B5F7B] dark:text-whiteOff  gap-10">
              <p className="inline text-start break-words font-semibold">
            ...
            <div className="flex text-[#5B5F7B] dark:text-whiteOff gap-10">
              ...
              <p className=" text-start font-normal inline">{salary?.rate}</p>
            </div>
            <div className="flex text-[#5B5F7B] dark:text-whiteOff gap-10">
              <p className="inline text-start break-words font-semibold">
                Comment:
