...
const Salary = () => {
  ...
  const [requestAddAllowances, setRequestAddAllowances] =
  ...
  const [editableAllowancesRow, setEditableAllowancesRow] =
    useState<Allowances | null>(null);
  const [editableHolidaysRow, setEditableHolidaysRow] =
    useState<Holidays | null>(null);
  const typeList = [
  ...
  const allowancesColumnsForEmployee: ColumnType[] = [
    {
      title: "No",
      type: ColumnEnum.indexColumn,
      key: "no",
  ...
  const holidaysColumnsForEmployee: ColumnType[] = [
    {
      title: "No",
      type: ColumnEnum.indexColumn,
      key: "no",
  ...
  const onSubmitHolidays = async (updatedRow: Holidays) => {
    try {
      ...
      toast({
        ...
        description: [`name: ${updatedRow.name}\n`, `day: ${updatedRow.day}`],
      });
  ...
  const AddAllowancesModal: React.FC<AddAllowancesModalProps> = ({
    onClose,
    onSave,
  }) => {
    ...
    return (
      <div className="flex flex-1 flex-col bg-white dark:bg-dark w-full min-h-unit-3 items-start pt-8 pb-20 mb-8 px-28 gap-4 text-[#5B5F7B] text-sm shadow-[0_4px_4px_0px_rgba(0,0,0,0.25)] rounded-lg">
        ...
        <div className="flex gap-3 self-end mb-2">
          <RegularButton
            label="Submit"
            callback={() => onSave(formState as Allowances)}
          />
          <RegularButton
            label="Close"
  ...
  const AddHolidaysModal: React.FC<AddHolidaysModalProps> = ({
    onClose,
    onSave,
  }) => {
    ...
    return (
      <div className="flex flex-1 flex-col bg-white  dark:bg-dark w-full min-h-unit-3 items-start pt-8 pb-20 mb-8 px-28 gap-4 text-[#5B5F7B] text-sm shadow-[0_4px_4px_0px_rgba(0,0,0,0.25)] rounded-lg">
        ...
        <div className="flex gap-3 self-end mb-2">
          <RegularButton
            label="Submit"
            callback={() => onSave(formState as Holidays)}
          />
          <RegularButton
            label="Close"
            additionalStyle="bg-[#BDBDBD]"
  ...
  const handleEdit = (id: string) => {
    switch (type) {
      case "allowances":
        const rowForEditAllowances =
          allowances?.find((allowance) => allowance._id === id) || null;
        setEditableAllowancesRow(rowForEditAllowances);
        break;
      case "holidays":
        const rowForEditHolidays =
          holidays?.find((holiday) => holiday._id === id) || null;
  ...
  const EditAllowancesModal: React.FC<EditAllowancesModalProps> = ({
    row,
    onClose,
    onSave,
  }) => {
  ...
  const EditHolidaysModal: React.FC<EditHolidaysModalProps> = ({
    row,
    onClose,
    onSave,
  }) => {
  ...
  const deleteFunction = async (id: string) => {
    switch (type) {
      ...
      case "holidays":
        try {
          const res = await axiosPrivate.delete<Holidays>("/holiday/" + id, {
            headers: { "Content-Type": "application/json" },

            withCredentials: true,
          });
          console.log({ res });
          const updateHolidays = holidays.filter(
