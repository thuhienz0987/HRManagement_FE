...
const AbsentDetail = ({
    ...
    const basicInformation = [
        ...
        !session?.user.roles.includes(process.env.CEO)?{ label: "Department:", value: profile?.departmentId?.name }:undefined,
    ...
    const isCurrentUser = (leaveRequestId?: string, leaveRequestStatus?: string) => {
        if(leaveRequestId === session?.user._id && leaveRequestStatus === "pending")
            return true;
        else
            return false;
    };
    ...
    return (
        <div className="flex flex-1 flex-col px-[4%] items-center pb-4">
            ...
            <div className="flex flex-1 flex-col bg-white w-full min-h-unit-3 items-start pt-8 pb-20 px-28 gap-4 text-[#5B5F7B] text-sm shadow-[0_4px_4px_0px_rgba(0,0,0,0.25)] rounded-lg">
                ...
                <RegularButton label="Save" callback={handleSubmit} isLoading={isLoading}/>
                </div>
            </div>
            </>
            ):(
            <>
            <div className="flex gap-3 self-end mb-2">
                {leaveRequest?.status === "pending" && session?.user.roles.includes(process.env.HRManager) && (
                    <>
                        <RegularButton
                            label="Approve"
                            callback={handleApprove}
