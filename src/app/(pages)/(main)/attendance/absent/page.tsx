...
const Absent = () => {
  ...
  useEffect(() => {
    const getLeaveRequest = async () => {
      try {
        const res = await axiosPrivate.get<dLeaveRequest[]>("/leaveRequests");
        res.data.map((lr) => {
    ...
    if (session?.user.roles.includes(process.env.HRManager)) getLeaveRequest();
  ...
  return (
    <div className="flex flex-col">
      <div className="flex flex-col w-11/12 self-center gap-9 pb-8">
        <div className="flex bg-white dark:bg-dark w-full gap-5 shadow-[0_4px_4px_0px_rgba(0,0,0,0.25)] rounded-lg p-7">
          <TableFirstForm
