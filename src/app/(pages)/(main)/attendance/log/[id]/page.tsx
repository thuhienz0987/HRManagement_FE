        ...
        const getEmployees = async (month: number, year: number) => {
            try {
                ...
                res.data.map((atd) => {
                    ...
                    atd.arriveTime = format(
                        parseISO(atd.checkInTime),
                        "kk:mm:ss"
                    );
                    let [hours, minutes, seconds] = atd.arriveTime
                        .split(":")
                        .map(Number);
                    ...
                    if (atd.checkOutTime) {
                        atd.leaveTime = format(
                            parseISO(atd.checkOutTime),
                            "kk:mm:ss"
                        );
        ...
        return (
            <div className="flex flex-1 flex-col bg-white w-full min-h-unit-3 items-start pt-8 pb-20 mb-8 px-28 gap-4 text-[#5B5F7B] text-sm shadow-[0_4px_4px_0px_rgba(0,0,0,0.25)] rounded-lg">
                <div className="flex w-full items-start py-10 gap-48">
                    ...
                    <Input
                        value={format(
                            new Date(formState.attendanceDate),
                            "dd/MM/yyyy"
                        )}
