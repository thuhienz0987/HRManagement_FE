...
const DailyAttendance = () => {
    ...
    const [editableRow, setEditableRow] = useState<dDailyAttendances | null>(null);
        ...
        const getAttendancesByDate = async (day: number, month: number, year: number) => {
            try {
                ...
                res.data.map((attendance) => {
                    ...
                    let [hours, minutes, seconds] = attendance.arriveTime.split(':').map(Number);
                    ...
                    if(attendance.checkOutTime) 
                    {
                        ...
                        let [hours, minutes, seconds] = attendance.leaveTime.split(':').map(Number);
    ...
    const onSave = async (updatedRow: dDailyAttendances) => {
        try {
            const [arriveTimeHours, arriveTimeMinutes, arriveTimeSeconds] = updatedRow.arriveTime.split(':');
            ...
            const [leaveTimeHours, leaveTimeMinutes, leaveTimeSeconds] = updatedRow.leaveTime.split(':');
            ...
            if((arriveTime.getHours() <= 7 
            || (arriveTime.getHours() === 7 && arriveTime.getMinutes() === 0 && arriveTime.getSeconds() === 0)) 
    ...
    const handleEdit = (id: string) => {
        const rowForEdit = dailyAttendances?.find((attendance) => attendance._id === id) || null;
            ...
            <div className="flex flex-1 flex-col bg-white w-full min-h-unit-3 items-start pt-8 pb-20 mb-8 px-28 gap-4 text-[#5B5F7B] text-sm shadow-[0_4px_4px_0px_rgba(0,0,0,0.25)] rounded-lg">
                ...
                <div className="flex w-full items-start py-10 gap-48">
                    <Input 
                        ...
                        labelPlacement={"outside"}
                        onChange={(e) => setFormState({ ...formState, arriveTime: e.target.value })}
                    />
                    <Input 
                        ...
                        labelPlacement={"outside"}
                        onChange={(e) => setFormState({ ...formState, leaveTime: e.target.value })}
                    />
                </div>
        ...
        <div className="flex flex-1 flex-col px-[4%] items-center pb-4 rounded">
            ...
            <div className="flex flex-1 flex-col bg-white w-full min-h-unit-3 items-start py-16 gap-5 shadow-[0_4px_4px_0px_rgba(0,0,0,0.25)] rounded-lg ">
                ...
                <div className="w-[95%] self-center flex">
                    ...
                    editFunction={handleEdit} deleteFunction={handleDelete}/>
                </div>
            </div>
        </div>
    );
