...
const ContentBody: React.FC<ContentBodyProps> = ({ selectedUser }) => {
  ...
  useEffect(() => {
    if (selectedUser && session?.user._id) {
      ...
      socket?.on("loadNewChat", ({chat}) => {
        ...
        if (
          (chat.senderId === session.user._id && chat.receiverId === selectedUser._id) ||
          (chat.senderId === selectedUser._id && chat.receiverId === session.user._id)
        ) {
  ...
  return (
    <div dir="btt" className="flex flex-col flex-1 relative overflow-y-scroll">
